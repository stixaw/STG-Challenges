    ...
    describe('Most Popular Searches', () => {
        ...
        it('should get href for each popular search using while loop and print to console', async () =>{
            ...
            while(count < popular_array.length){
                var carModel = await popular_array[count].getText()
                var url = await popular_array[count].getAttribute('href')
                console.log(carModel + " - " + url)
                count ++
            }
        })

        //for loop:
        it('should get href for each trending search using for loop and print to console', async () => {
            ...
            for(var i = 0; i < popular_array.length; i++){
                ...
                var url = await popular_array[i].getAttribute('href')
